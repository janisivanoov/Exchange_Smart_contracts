...
describe("OuroborusRouter", function () {
  let erc721;
  ...
  before(async () => {
    ...
    const erc721MetaTransactionMaticSample = await MetaTransaction.deploy(
      "Sample Token",
      "ST"
    );
...
contract('Tests', () => {
  it('', () =>
    Tests.deployed().then((inst) => inst.runTests({gas: '10000000'})))
})